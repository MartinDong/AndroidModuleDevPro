...
public class SimpleMonthAdapter extends RecyclerView.Adapter<SimpleMonthAdapter.ViewHolder> implements SimpleMonthView.OnDayClickListener {
     ...
     * 是否可以选择同一天
     */
    private Boolean mCanSelectSameDay;
    ...
    private void initData() {
        ...
        mCanSelectSameDay = dataModel.canSelectSameDay;
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        drawingParams.put(SimpleMonthView.Companion.getVIEW_PARAMS_SELECTED_BEGIN_DATE(), rangeDays.getFirst());
        drawingParams.put(SimpleMonthView.Companion.getVIEW_PARAMS_SELECTED_LAST_DATE(), rangeDays.getLast());
        drawingParams.put(SimpleMonthView.Companion.getVIEW_PARAMS_NEAREST_DATE(), mNearestDay);
    ...
    public void setRangeSelectedDay(CalendarDay calendarDay) {
        ...
        if (rangeDays.getFirst() != null && rangeDays.getLast() == null) {
            ...
            if (!mCanSelectSameDay) {
                // 所选退房日期不能与入住日期相等
                if (calendarDay.getDate().equals(rangeDays.getFirst().getDate())) {
                    if (mController != null) {
                        mController.alertSelectedFail(DatePickerController.FailEven.BEGIN_CANT_EQUALS_END);
                    }
                    return;
                }
            }

            // 所选退房日期不能再入住日期之前，如果选择的是之前的可选日期，那么需要重置入住时间，并刷新界面。
