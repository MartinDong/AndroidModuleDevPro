...
import com.dong.lib.common.di.component.DaggerAppComponent;
import com.dong.lib.common.di.module.GlobalConfigModule;
import com.dong.lib.common.integration.ConfigModule;
import com.dong.lib.common.integration.ManifestParser;
...
public class AppDelegate implements App, AppLifecycle {
    ...
    public AppDelegate(Context context) {
        ...
        for (ConfigModule module : mModules) {
            ...
            //将框架外部, 开发者实现的 Application 的生命周期回调 (AppLifecycles) 存入 mAppLifecycleList 集合 (此时还未注册回调)
            module.injectAppLifecycle(context, mAppLifecycleList);
    ...
    public void attachBaseContext(Context base) {
        ...
        //遍历 mAppLifecycleList, 执行所有已注册的 AppLifecycles 的 attachBaseContext() 方法 (框架外部, 开发者扩展的逻辑)
        for (AppLifecycle lifecycle : mAppLifecycleList) {
    ...
    public void onLowMemory(Application application) {
        if (mAppLifecycleList != null && mAppLifecycleList.size() > 0) {
            //遍历 mAppLifecycleList ,执行所有已注册的 AppLifecycle 的 onCreate() 方法
            for (AppLifecycle lifecycle :
                    mAppLifecycleList) {
